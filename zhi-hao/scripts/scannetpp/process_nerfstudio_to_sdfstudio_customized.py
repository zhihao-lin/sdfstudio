...
def main(args):
    ...
    cam_params = json.load(open(input_dir / "nerfstudio" / "transforms_undistorted.json"))
    ...
    for frame in frames:
        ...
        # load images
        img_path = os.path.join("undistorted_images", frame["file_path"])
    ...
    for idx, (valid, pose, image_path) in enumerate(tqdm(zip(valid_poses, poses, image_paths))):
        ...
        if args.mono_prior:
            ...
            frame["mono_depth_path"] = os.path.join("depth_omnidata", name.replace(".JPG", "_depth.npy"))
            frame["mono_normal_path"] = os.path.join("normal_omnidata", name.replace(".JPG", "_normal.npy"))
    ...
    out_path = str(output_dir / "meta_data.json")
    print("Saving meta data to", out_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input-dir", dest="input_dir", required=True, help="path to nerfstudio data directory")
